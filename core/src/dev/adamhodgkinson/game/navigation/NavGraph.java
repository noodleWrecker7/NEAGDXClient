...
public class NavGraph {
    ...
    public JumpArc addJumpEdge(short x1, short y1, short x2, short y2, short weight, float xSpeed, float jumpSpeed) {
        final int index1 = coordToIndexMap[x1][y1];
        final int index2 = coordToIndexMap[x2][y2];
    ...
    public Vertex getVertexByCoords(short x, short y) {
        int index = getNodeIndexByCoords(x, y);
        if (index == -1) {
            return null;
        }
    ...
    public int getNodeIndexByCoords(short x, short y) {
        if (x < 0 || y < 0)
            return -1;
        if (x >= coordToIndexMap.length || y >= coordToIndexMap[0].length)
            return -1;
    ...
    public Arc getArc(short fromX, short fromY, short toX, short toY) {
        int from = getNodeIndexByCoords(fromX, fromY);
        int to = getNodeIndexByCoords(toX, toY);
        if (from == -1 || to == -1) {
            return null;
        }
